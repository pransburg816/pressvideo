...
(function () {
	...
	var maxPages = parseInt(main.dataset.pvMaxPages, 10) || 1;
	if (maxPages <= 1 || document.querySelector('.pv-broadcast')) return;

	var baseUrl  = main.dataset.pvBaseUrl || window.location.href.replace(/[?#].*/, '');
	var curPage  = 1;
	var busy     = false;
	var allDone  = false;

	var sentinel = document.createElement('div');
	sentinel.className = 'pv-scroll-sentinel';

	var loader = document.createElement('div');
	loader.className = 'pv-scroll-loader';
	loader.innerHTML = '<span class="pv-scroll-spinner"></span>';

	var endMsg = document.createElement('p');
	endMsg.className = 'pv-scroll-end';
	endMsg.textContent = '\u2014 You\u2019ve seen everything \u2014';
	endMsg.hidden = true;

	var pagination = main.querySelector('.pv-pagination');
	if (pagination) {
		main.insertBefore(sentinel, pagination);
		main.insertBefore(loader, pagination);
		main.insertBefore(endMsg, pagination);
	...
	function loadMore() {
		...
		busy = true;
		loader.classList.add('pv-scroll-loader--active');

		var url = baseUrl.replace(/\/page\/\d+\/?/, '/') + '?paged=' + (curPage + 1);
		...
		fetch(url)
			...
			.then(function (html) {
				...
				var src = doc.querySelector('.pv-wall')
					|| doc.querySelector('.pv-spotlight-grid')
					|| doc.querySelector('.pv-list')
					|| doc.querySelector('.pv-grid');

				if (!src) { allDone = true; return; }

				var newItems = Array.from(src.children).filter(function (el) {
					return el.hasAttribute('data-category');
				});

				if (!newItems.length) { allDone = true; return; }

				var container = getContainer();
				if (!container) { allDone = true; return; }
				...
				requestAnimationFrame(function () {
					requestAnimationFrame(function () {
						...
						setTimeout(function () {
							newItems.forEach(function (item) { item.style.transitionDelay = ''; });
						}, Math.min(newItems.length * 40, 320) + 450);
					});
				});
			...
			.catch(function () { allDone = true; })
			.finally(function () {
				busy = false;
				loader.classList.remove('pv-scroll-loader--active');
			});
	}
