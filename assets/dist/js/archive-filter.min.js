...
(function () {
	...
	/* ── Music/Video type toggle ──────────────────────────────────────── */

	var pvTypeFilter = '*';

	function applyTypeFilter(container) {
		if (!container) return;
		var all = Array.from(container.querySelectorAll('.pv-card, .pv-bc-card'));
		var matching = [];
		all.forEach(function (item) {
			if (pvTypeFilter === '*') {
				...
				var isMusic = item.dataset.isMusic === '1';
				var show = pvTypeFilter === 'music' ? isMusic : !isMusic;
				item.style.display = show ? '' : 'none';
				if (show) matching.push(item);
			}
		});
		updateTypeChrome(container, matching.length);
	}

	// Hide sort/pagination/per-page when filtered result is small (< 24).
	// Uses data-pv-type-hide to track what we hid so we can restore correctly.
	function updateTypeChrome(container, matchCount) {
		var isFiltered = pvTypeFilter !== '*';
		var scope = (container && container.closest) ? (container.closest('.pv-bc-panel') || main) : main;
		if (!scope) return;
		// Exclude top-pagination (it's inside the toolbar which we already hide)
		var sel = '.pv-toolbar, .pv-bc-toolbar, .pv-pagination:not(.pv-pagination--top), .pv-sort-bar';
		if (isFiltered && matchCount < 24) {
			scope.querySelectorAll(sel).forEach(function (el) {
				if (!el.hidden) { el.dataset.pvTypeHide = '1'; el.hidden = true; }
			});
		} else {
			scope.querySelectorAll(sel).forEach(function (el) {
				if (el.dataset.pvTypeHide === '1') { el.hidden = false; delete el.dataset.pvTypeHide; }
			});
		}
	}

	// Hide the toggle entirely if no music items exist in the loaded set.
	function checkMusicToggleVisibility(container) {
		if (!container) return;
		var hasMusicItems = !!container.querySelector('.pv-card[data-is-music="1"], .pv-bc-card[data-is-music="1"]');
		var scope = (container && container.closest) ? (container.closest('.pv-bc-panel') || main) : main;
		if (!scope) return;
		scope.querySelectorAll('.pv-type-toggle').forEach(function (toggle) {
			toggle.hidden = !hasMusicItems;
		});
		// Reset filter to All if the selected type has no items
		if (!hasMusicItems && pvTypeFilter !== '*') {
			pvTypeFilter = '*';
			document.querySelectorAll('.pv-type-btn').forEach(function (b) {
				b.classList.toggle('pv-type-btn--active', b.dataset.type === '*');
			});
	...
	function initTypeToggles() {
		document.querySelectorAll('.pv-type-toggle').forEach(function (bar) {
			bar.addEventListener('click', function (e) {
				var btn = e.target.closest('.pv-type-btn');
				if (!btn) return;
				pvTypeFilter = btn.dataset.type || '*';
				document.querySelectorAll('.pv-type-btn').forEach(function (b) {
					b.classList.toggle('pv-type-btn--active', b.dataset.type === pvTypeFilter);
				});
				var container = getContainer();
				if (container) applyTypeFilter(container);
				var bcHomeGrid   = document.querySelector('.pv-bc-home-grid');
				var bcVideosGrid = document.querySelector('.pv-bc-video-grid');
				applyTypeFilter(bcHomeGrid);
				applyTypeFilter(bcVideosGrid);
			});
		});
		// Initial visibility check for static (non-AJAX) grids
		var container = getContainer();
		if (container) checkMusicToggleVisibility(container);
	}
		...
		function loadBcHomePlaylistGrid(plId, page, perPage) {
			...
			fetch(bcAjaxUrl, { method: 'POST', body: fd })
				...
				.then(function (data) {
					...
					applyBcHomeSort();
					applyTypeFilter(bcHomeGrid);
					checkMusicToggleVisibility(bcHomeGrid);
		...
		function loadBcVideosGrid(page, perPage, category, ytPlId) {
			...
			fetch(bcAjaxUrl, { method: 'POST', body: fd })
				...
				.then(function (data) {
					...
					applyTypeFilter(bcVideosGrid);
					checkMusicToggleVisibility(bcVideosGrid);
