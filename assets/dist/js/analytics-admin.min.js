...
(function () {
		...
		var pool = [
			{
				...
				edge: 'YouTube\'s algorithm rewards watch time above everything else. Your ' + comp + '% avg completion tells us viewers are leaving before they hit your best content. A stronger 15-second hook on your top video alone can lift the whole channel’s recommendation score.',
				...
				impact: '+10–20% avg watch time, which directly increases YouTube’s recommendation push.',
			},
			{
				...
				edge: 'Email subscribers are 5–10× more likely to watch than cold traffic. Creators who send a “here’s what you missed” email after a video goes up see 30–50% more plays from their existing list within 48 hours — with zero ad spend.',
			...
			},
			{
				...
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>',
				title: 'Test a new thumbnail on “' + truncStr(top3vid.title, 38) + '”',
				edge: 'Thumbnail CTR is the #1 factor in how many new people discover your content. A video with a mediocre thumbnail can be sitting on gold. Changing one element — the emotion, the text, the color contrast — often doubles CTR overnight.',
				...
				impact: '2–5× click-through rate uplift, which multiplies YouTube impressions directly.',
			},
			{
				...
				edge: 'Your plays are up ' + trendPct + '% in the second half of this period. This is exactly when most creators go quiet. The ones who double down right now — more shares, more engagement, more posts — are the ones who break through to the next level.',
			...
			},
			{
				...
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>',
				title: 'Build a SEO hub page for “' + truncStr(top1.title, 35) + '”',
				edge: 'Unlike YouTube where content disappears in the feed, your video lives permanently on your site. A dedicated page with a keyword-optimized title, meta description, and transcript can rank in Google and drive organic traffic — something YouTube-only creators can never do.',
			...
			},
			{
				...
				edge: 'YouTube rewards channels that post consistently. Channels on a regular schedule see 2–3× more impressions than sporadic ones — even at the same video quality. One month of consistency can permanently lift your baseline discovery rate.',
				...
				impact: '2–3× baseline impressions for channels that post on a consistent schedule for 4+ weeks.',
			},
		];
	...
	function renderCoach(data) {
		...
		var playbook = generatePlaybook(data);
		var score    = edge.total;

		var scoreColor = score >= 75 ? '#10b981' : score >= 50 ? '#818cf8' : '#f59e0b';
		var narrative  = score >= 75
			? 'You’re outperforming most YouTube-only creators. Your content lives on your own site — indexed, brandable, and owned by you.'
			: score >= 50
			? 'You’re ahead of the curve. Most creators never get their video content indexed and searchable outside YouTube.'
			: 'You’re building a foundation most creators never have. Every video you publish is permanent, SEO-able, and owned by you.';
		...
		var html = '<div class="pva-insights-section pva-coach">'
			+ '<div class="pva-insights-header">'
			+   '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 3L1 9l11 6 9-4.91V17h2V9L12 3zM5 13.18v4L12 21l7-3.82v-4L12 17l-7-3.82z"/></svg>'
			+   '<h3>Creator Growth Coach</h3>'
			+   '<p>Your personalized competitive edge analysis + 3-move playbook</p>'
			+ '</div>'
			+ '<div class="pva-coach__body">'
			+   '<div class="pva-coach__gauge-col">'
			...
			+     '<h4 class="pva-coach__playbook-title">Your 3-Move Growth Playbook</h4>'
			+     '<p class="pva-coach__playbook-sub">Data-driven scripts personalized to your content — copy and use today</p>'
			+     moveCards
			+   '</div>'
			+ '</div>'
			+ '</div>';
	...
	function renderInsights(data) {
		...
		var coachHtml = renderCoach(data);

		if (!insights.length && !picks.length && !coachHtml) { section.hidden = true; return; }
		...
		section.innerHTML = coachHtml + picksHtml + insightsHtml;
		section.hidden    = false;

		// Animate SVG arc gauge
		var arc = section.querySelector('.pva-coach__arc');
		if (arc) {
		...
		// Wire copy-script buttons
		section.querySelectorAll('.pva-coach-copy').forEach(function(btn){
			btn.addEventListener('click', function(){
