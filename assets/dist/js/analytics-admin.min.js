...
(function () {
	...
	function exitFocusMode() {
		...
		document.querySelectorAll(CONTENT_ROWS).forEach(function(row) { row.style.display = ''; });
		var row = focusedCard.closest('.pva-inner > *');
		if (row) {
			row.classList.remove('pva-focus-row');
			Array.from(row.children).forEach(function(child) { child.style.display = ''; });
		}
		focusedCard.classList.remove('pva-focus-card');
		var btn = focusedCard.querySelector('.pva-expand-btn');
		if (btn) {
	...
	function enterFocusMode(card) {
		...
		if (row) {
			row.classList.add('pva-focus-row');
			Array.from(row.children).forEach(function(child) {
				if (child !== card && !child.contains(card)) child.style.display = 'none';
			});
		}
	...
	function render(data) {
		...
		// focus mode is transient — no restore needed
	}
