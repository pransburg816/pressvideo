...
(function () {
	...
	function render(data) {
		...
		var gridLower     = document.getElementById('pva-grid-lower');

		if (chartsSection) chartsSection.hidden = !hasData;
		if (emptySection)  emptySection.hidden  = hasData;
		if (gridLower)     gridLower.hidden     = !hasData;
	...
	function renderCoach(data) {
		...
		var UP_ICO  = '<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/></svg>';
		var DN_ICO  = '<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M16 18l2.29-2.29-4.88-4.88-4 4L2 7.41 3.41 6l6 6 4-4 6.3 6.29L22 12v6z"/></svg>';
		var VID_ICO = '<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg>';
		var benches = [
		...
		coachCol.innerHTML = '<div class="pv-card pva-coach">'
			...
			+     '<p class="pva-coach__playbook-sub">Data-driven scripts personalized to your content</p>'
			+     moveCards
			+   '</div>'
			+ '</div>'
			+ '</div>';
	...
	function renderSummary(data) {
		...
		// ── Grade + headline + body ───────────────────────────────────
		var grade, gradeColor, headline, body;
		...
		if (plays === 0) {
			...
			body       = 'You\'re trending upward with ' + fmt(plays) + ' plays this period and a ' + completion + '% average completion rate. Viewers are engaging — now is the time to promote your best content harder and publish more consistently.';
		} else if (trendDir === 'down') {
			grade      = 'Needs Attention';
			gradeColor = '#ef4444';
			headline   = 'Plays are sliding — here\'s how to reverse it';
			body       = 'Your plays trended down this period. With a ' + completion + '% average completion, the audience that finds your content does engage — the gap is visibility, not quality. A single targeted share or featured post placement can reset the curve.';
		} else {
			grade      = 'Holding Steady';
			gradeColor = '#0ea5e9';
			headline   = 'Consistent viewership — time to scale it';
			body       = fmt(plays) + ' plays with a ' + completion + '% average completion rate. Your audience is engaged and returning. The next unlock is expanding reach — bring new viewers into the funnel and let the content do the rest.';
		}
		...
		if (completion < 25) {
			tips.push({
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2.05v2.02c3.95.49 7 3.85 7 7.93 0 3.21-1.81 6-4.72 7.72L13 17v5h5l-1.78-1.78C19.14 18.45 22 15.3 22 12c0-5.18-3.94-9.45-9-9.95zM11 2.05C5.95 2.55 2 6.82 2 12c0 3.3 1.61 6.23 4.11 8.07L4.5 21.5H9v-5.02L6.97 18.7C5.09 17.22 4 15.19 4 12c0-4.08 3.05-7.44 7-7.93V2.05z"/></svg>',
				color: '#ef4444',
			...
			tips.push({
				...
				desc: 'Viewers pass the intro but drop before halfway. Review your videos between the 30% and 50% marks — that\'s where slow, repetitive, or low-value sections kill retention.',
			});
			...
			tips.push({
				...
				desc: completion + '% average completion is excellent. Place a strong CTA in your final 30 seconds — subscribe, visit your site, join your list — while viewer attention and trust are at their peak.',
			});
		}
		...
		if (trendDir === 'down') {
			tips.push({
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg>',
				color: '#ef4444',
				title: 'Re-promote your top performers',
				desc: 'Plays are trending down. Reshare your best 2-3 videos in email or social — returning traffic is free and often restarts growth faster than publishing something new.',
			});
		} else if (trendDir === 'up') {
			tips.push({
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/></svg>',
			...
			tips.push({
				icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M3.5 18.49l6-6.01 4 4L22 6.92l-1.41-1.41-7.09 7.97-4-4L2 16.99z"/></svg>',
				color: '#0ea5e9',
		...
		if (plays > 0 && videos > 0) {
			if (plays / videos < 5) {
				tips.push({
					icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-8 12.5v-9l6 4.5-6 4.5z"/></svg>',
					color: '#8b5cf6',
					title: 'Focus your promotion',
					desc: 'Plays are spread thin across ' + fmt(videos) + ' videos. Pick your top 2 performers and actively promote them — concentrated traffic builds social proof and momentum faster.',
				});
			} else {
				tips.push({
					icon: '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>',
		...
		section.innerHTML = '<div class="pva-summary">'
			...
			+     '<span class="pva-summary__grade" style="--grade-clr:' + gradeColor + '">'
			+       '<span class="pva-summary__grade-dot"></span>'
			+       esc(grade)
			+     '</span>'
			+   '</div>'
	...
	function renderInsights(data) {
		...
		var picksCard = '<div class="pv-card">'
			+ '<div class="pv-card__head">'
			+   '<h2 class="pv-card__title">What to Feature</h2>'
			+   '<span class="pva-chart-sub">Top content to drive engagement now</span>'
			+ '</div>'
			+ '<div class="pv-card__body">'
			+   (pickCards ? '<div class="pva-picks-grid">' + pickCards + '</div>' : '<p class="pva-no-data" style="padding:28px 22px;">No featured content for this period.</p>')
			+ '</div>'
			+ '</div>';

		var insightsCard = '<div class="pv-card">'
			+ '<div class="pv-card__head">'
		...
		section.innerHTML = '<div class="pva-insights-grid">' + picksCard + insightsCard + '</div>';
		section.hidden    = false;
	}
