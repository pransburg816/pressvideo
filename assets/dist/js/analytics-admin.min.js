...
(function () {
	...
	function renderYouTubeCoach(ytData) {
		...
		coachCol.innerHTML = '<div class="pv-card pva-coach" data-card-id="coach">'
			...
			+     '</svg>'
			+     '<div class="pva-coach__gauge-aside">'
			+       '<div class="pva-summary__grade-badge" style="--grade-clr:' + letterGrade.color + ';width:44px;height:44px;border-radius:10px;display:flex;align-items:center;justify-content:center;margin-bottom:8px">'
			+         '<span class="pva-summary__grade-letter" style="font-size:1.3rem">' + letterGrade.letter + '</span>'
			+       '</div>'
			+       '<p class="pva-coach__narrative">' + esc(narrative) + '</p>'
			+     '</div>'
			+   '</div>'
	...
	function switchToSite() {
		...
		// Restore depth chart canvas (replaced by traffic sources on YT tab).
		var depthCtx = document.getElementById('pva-depth-chart');
		if (depthCtx) depthCtx.style.display = '';
		var depthWrap = depthCtx ? depthCtx.parentNode : null;
		if (depthWrap) {
			var srcList = depthWrap.querySelector('.pva-sources-list');
	...
	function renderYtInSources(sourcesData) {
		...
		var wrap = ctx ? ctx.parentNode : null;
		if (!wrap) return;

		var existing = wrap.querySelector('.pva-sources-list');
		if (existing) existing.remove();
		var existingNd = wrap.querySelector('.pva-no-data');
		if (existingNd) existingNd.remove();
		...
		if (!srcs.length || !total) {
			...
			nd.className = 'pva-no-data';
			nd.textContent = 'No traffic source data for this period.';
			wrap.appendChild(nd);
			return;
		}
