...
(function () {
	...
	var CONTENT_ROWS = '.pva-ai-row, .pva-charts-row, .pva-table-row, #pva-feature-section, #pva-summary-section';
	var FOCUS_MS     = 220;
	...
	function renderYouTubeCoach(ytData) {
		...
		// Score: weighted average of retention (40%), engagement (40%), sub conversion (20%)
		...
		var grade      = score >= 70 ? 'Strong Channel' : score >= 45 ? 'Growing' : score >= 25 ? 'Building' : 'Getting Started';
		...
		var playbook = generateYtPlaybook(ytData);
		var moveCards = playbook.map(function(move, i) {
			...
			return '<div class="pva-coach-move" style="--move-clr:' + move.color + '">'
				...
				+   '<div class="pva-coach-script" id="' + sid + '">' + esc(move.script) + '</div>'
				+   '<button class="pva-coach-copy" data-target="' + sid + '" type="button">'
				+     '<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg> Copy script'
				+   '</button>'
				+ '</div>'
				+ '<p class="pva-coach-move__impact"><strong>Expected impact:</strong> ' + esc(move.impact) + '</p>'
				+ '</div>';
		...
		var dashOffset = Math.round(251 - (score / 100) * 251);
		var letterGrade = score >= 90 ? { letter: 'A', color: '#10b981' }
			: score >= 75 ? { letter: 'B', color: '#6366f1' }
			: score >= 55 ? { letter: 'C', color: '#f59e0b' }
			: score >= 35 ? { letter: 'D', color: '#f97316' }
		...
		coachCol.innerHTML = '<div class="pv-card pva-coach" data-card-id="coach">'
			...
			+ '</div>'
			+ '<div class="pv-card__body pva-coach__body">'
			+   '<div class="pva-coach__grade-row">'
			+     '<span class="pva-coach__grade-name">' + esc(grade) + '</span>'
			+     '<span class="pva-coach__grade-score">Score: ' + score + ' / 100</span>'
			+   '</div>'
			+   '<div class="pva-coach__benchmarks">' + subBars + '</div>'
			+   '<div class="pva-coach__gauge-row">'
			+     '<svg class="pva-coach__gauge" viewBox="0 0 200 110" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
			+       '<path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke="#e5e7eb" stroke-width="14" stroke-linecap="round"/>'
			+       '<path class="pva-coach__arc" d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke="' + scoreColor + '" stroke-width="14" stroke-linecap="round"'
			+         ' stroke-dasharray="251" stroke-dashoffset="' + dashOffset + '" style="transition:stroke-dashoffset 1s ease"/>'
			+       '<text x="100" y="95" text-anchor="middle" font-size="28" font-weight="700" fill="' + scoreColor + '">' + score + '</text>'
			+     '</svg>'
			+     '<div class="pva-coach__gauge-aside">'
			+       '<p class="pva-coach__narrative">' + esc(narrative) + '</p>'
			+       '<div class="pva-coach__subbars">' + subBars + '</div>'
			...
			+   '</div>'
			+   '<div class="pva-coach__playbook">'
			+     '<h4 class="pva-coach__playbook-title">Your YouTube Growth Playbook</h4>'
			+     '<p class="pva-coach__playbook-sub">3 moves ranked by impact. Start with move #1.</p>'
			+     moveCards
			+   '</div>'
			+ '</div>'
			+ '</div>';
	...
	function renderYouTube(data) {
		...
		// YouTube-specific AI coach + insights
